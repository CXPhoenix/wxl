...
- [ ] 6.1 在 `chall-wasm/wxlsh-parser/src/lib.rs` 實作 `parse_command(input: &str) -> ParsedCommand`，處理 quoted string、`--flag=value`、`-f value` 語法，以實作 wxlsh uses Rust WASM for command parsing
- [ ] 6.2 在 `commands` 模組加入 Rust-native 命令：`help`（列出所有命令）、`clear`（清空終端）、`base64`、`hex`，以實作 wxlsh supports Rust-native command extension
- [ ] 6.3 撰寫 wxlsh-parser 的 Rust unit tests，覆蓋 tokenize 引號、flag shorthands、空輸入

## 7. useWxlsh 組合函式

- [ ] 7.1 建立 `.vitepress/theme/composables/useWxlsh.ts`，實作命令 registry（Rust-native 優先查找，其次 Python-backed），以實作 wxlsh dispatches commands via Python (Pyodide) by default 中的 wxlsh 架構（Option B）設計決策
- [ ] 7.2 在 useWxlsh 中載入 Python-backed 命令集（curl-like HTTP、decode、encode）至 Pyodide，HTTP 命令透過 `dispatch()` prop 路由
- [ ] 7.3 實作命令歷史管理（in-memory + `appendHistory` 寫入 IDB、`loadHistory` 還原），以實作 wxlsh terminal history navigable with arrow keys

## 8. WxlshPanel.vue 元件

- [ ] 8.1 建立 `.vitepress/theme/components/WxlshPanel.vue`，使用 xterm.js 作為顯示層，在 `onMounted` 中初始化（lazy），在 `onUnmounted` 中 dispose，以實作 wxlsh terminal renders using xterm.js
- [ ] 8.2 顯示啟動 banner："wxlsh 1.0 — web exploit shell\ntype 'help' for available commands"
- [ ] 8.3 接入 `useWxlsh` 組合函式，將 xterm.js key input 連接至命令執行
- [ ] 8.4 實作 Up/Down 箭頭鍵歷史導航（xterm.js key event handler）
- [ ] 8.5 撰寫 WxlshPanel.vue 的單元測試，覆蓋：banner 顯示、lazy load、unmount cleanup、unknown command 錯誤訊息

## 9. CodeEditorPanel.vue 元件

- [ ] 9.1 建立 `.vitepress/theme/components/CodeEditorPanel.vue`，使用 CodeMirror 6 + `@codemirror/lang-python` + `@codemirror/autocomplete`，lazy init in `onMounted`，以實作 Code Editor Panel provides a Python editing environment with CodeMirror
- [ ] 9.2 實作上下可調整大小的 vertical split layout（drag handle、最小高度 120px/80px、預設 65/35），以實作 Code Editor Panel has a resizable vertical split layout
- [ ] 9.3 實作 "Run" 按鈕與 Ctrl+Enter 快捷鍵，透過 Pyodide `runPythonAsync` 執行程式碼；注入 `requests` stub 路由至 `dispatch()`，以實作 Code Editor Panel executes Python via Pyodide
- [ ] 9.4 實作 "Save" 按鈕（prompt 輸入名稱 → `saveScript`）與 "Load" 下拉選單（`listScripts` → 選取 → `loadScript`），以實作 Code Editor Panel supports named script save and load via IndexedDB
- [ ] 9.5 撰寫 CodeEditorPanel.vue 的單元測試，覆蓋：Python syntax highlight 初始化、Run 按鈕觸發執行、disabled overlay、Save/Load 流程

## 10. RepeatPanel UI 升級

- [ ] 10.1 重構 `RepeatPanel.vue` layout：將 snapshot list 從底部 inline chips 改為右側 sidebar named list，以實作 Repeater Panel provides raw HTTP request editing（RepeatPanel UI 升級設計決策）
- [ ] 10.2 "Save" 按鈕改為 prompt 輸入快照名稱後儲存（named snapshot can be saved and restored）
- [ ] 10.3 統一字體、border、line-height 等視覺細節，符合 challenge UI 色彩規範
...
- [ ] 11.8 確認 Challenge layout includes a navigation bar with a back link to the challenge list 仍正確顯示
- [ ] 11.9 套用 UnoCSS utility classes 至 WxlshPanel 與 CodeEditorPanel，確認 Challenge UI components use UnoCSS utility classes for styling；驗證新元件符合 Challenge UI applies the platform color palette（CodeMirror Code Editor Panel 與 IndexedDB 持久化（useChallengePersistence）新元件均使用 `--ch-*` design tokens）

## 12. 最終驗證

- [ ] 12.1 執行 `pnpm wasm:build`，確認 wxlsh-parser WASM 編譯無錯誤
- [ ] 12.2 執行 `pnpm test`（unit tests），確認所有測試通過
