...
**Reason**: Layout approach replaced by globally registered Vue component embedded in markdown, which is more flexible and conforms to VitePress conventions.
**Migration**: Remove `layout: challenge-list` from `docs/challenges/index.md` frontmatter and add `<ChallengeList />` to the page body instead.

#### Scenario: challenge-list layout is no longer registered

- **WHEN** the theme initializes
- **THEN** `theme/index.ts` SHALL NOT register any layout named `challenge-list`, and `ChallengeListLayout.vue` SHALL NOT exist
