...
- **THEN** the `PhpRuntime` class SHALL set up PHP superglobals (`$_SERVER`, `$_GET`, `$_POST`, `$_COOKIE`), execute the app's PHP file, and capture the output buffer as the response body

#### Scenario: PHP headers are forwarded in response

- **WHEN** the PHP script calls `header('Content-Type: application/json')`
- **THEN** the `PhpRuntime` SHALL include the header in the returned response descriptor
